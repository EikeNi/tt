...
def action_summary(month, year):
    ...
    for name in sorted(report.keys()):
        report[name]['rate'] = round(100 * report[name]['work_duration'] / target_working_time, 1)
        print(name.ljust(fill), ': ', "%5.1f"%report[name]['rate'], ' - TAGS:', sorted(list(set(report[name]['tags']))))
    print('-'*fill)
    print("Total".ljust(fill), ': ', "%5.1f"%sum([entry['rate'] for entry in report.values()]))
